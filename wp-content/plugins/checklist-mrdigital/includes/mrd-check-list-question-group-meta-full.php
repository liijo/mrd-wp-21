		...
		echo '<tr>
				...
				switch($field['type']) {
					...
					case 'textarea':
						...
						echo '<textarea style="width:100%" name="'.$field['id'].'" id="'.$field['id'].'" rows="5">'.$meta.'</textarea><br /><span class="description">'.$field['desc'].'</span>';
					break;
...
function add_mrc_checklist_qgroup_meta() {
	add_meta_box(
		't_csldr_meta_box', // $id
		'Questions', // $title
		'show_mrc_checklist_qgroup_meta', // $callback
		'mrd_question_group', // $page
		'normal', // $context
		'high' // $priority
	);
}
add_action('add_meta_boxes', 'add_mrc_checklist_qgroup_meta');
function show_mrc_checklist_qgroup_meta() {
	global $post;
  ...
  <div class="col-sm-6">
    ...
    $html .='<label for="">Question</label>
				<input type="text" name="wpnbr_slider[%1$d][q_title]" value="%4$s" placeholder="Question" class="form-control" />
	</div>
</div>
<div class="col-sm-6">
		<div class="form-group">';
...
<div class="col-sm-6">
	<div class="form-group">';
		$html .='<label for="">Description</label>
				<textarea name="wpnbr_slider[%1$d][q_description]" placeholder="Description&hellip;" class="form-control">%4$s</textarea>
	</div>
</div>
<div class="col-sm-6">
		<div class="form-group">';
		$html .='<label for="">Useful Links</label>
				 <textarea name="wpnbr_slider[%1$d][q_links]" placeholder="Useful Links" class="form-control">%4$s</textarea>
		</div>
	</div>
</div></li>';
      ...
      <div class="col-sm-6">
        <div class="form-group">';
        	echo '<label for="">Question</label>
							<input type="text" name="wpnbr_slider['.$i.'][q_title]" value="'.$item['q_title'].'" placeholder="Question" class="form-control" />
				</div>
      </div>
			<div class="col-sm-6">
					<div class="form-group">';
      ...
      <div class="col-sm-6">
        <div class="form-group">';
        	echo '<label for="">Description</label>
							<textarea name="wpnbr_slider['.$i.'][q_description]" placeholder="Description&hellip;" class="form-control">'.$item['q_description'].'</textarea>
				</div>
      </div>
			<div class="col-sm-6">
					<div class="form-group">';
        	echo '<label for="">Links</label>
							 <textarea name="wpnbr_slider['.$i.'][q_links]" placeholder="Usefull Links" class="form-control">'.$item['q_links'].'</textarea>
        	</div>
				</div>
      </div>
...
function save_mrc_checklist_qgroup_meta($post_id) {
	// verify nonce
	...
	foreach ( $slides as $slide ) {
		$q_title = trim($slide['q_title']);
		$q_point = trim($slide['q_point']);
		$q_description = trim($slide['q_description']);
		$q_links = trim($slide['q_links']);


		$new[] = array('q_title' => $q_title, 'q_point'=>$q_point, 'q_description'=>$q_description, 'q_links'=>$q_links);
	}

	update_post_meta($post_id, 'wpnbr_slider', $new);
}
add_action('save_post', 'save_mrc_checklist_qgroup_meta');
//Script
...
//
function mrc_checklist_qgroup_head() {
	?><style type="text/css">
    ...
    .wpnbr_slider li textarea{
      height:80px;
		...
		.wpnbr_slider li .hndle { left: -6px; line-height: 24px; background-color:#fff !important; border:solid 1px #C57173}
		...
		.wpnbr_slide_preview_image { display: block; margin:auto;  margin-bottom: 8px !important; border: none !important;}
...
add_action('admin_head-post-new.php', 'mrc_checklist_qgroup_head', 100);
add_action('admin_head-post.php', 'mrc_checklist_qgroup_head', 100);
?>
